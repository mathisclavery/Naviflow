...
# 2. Mise en forme X / y
# --------------------------------------------------------------------------- #
def prepare_xgb(df, lags=(1, 7, 30), horizon=None, keep_id=False, scale=False):
    """Prepare X (2D) et y pour un modele tabulaire.

    Parametres
    ----------
    df : DataFrame enrichi issu de feature_engineering.build_features().
    lags : decalages a creer. () ou None pour ne pas creer de lags.
    horizon : si renseigne (ex. 7), cree target_j{horizon} et l'utilise comme y.
              Si None, y = TARGET (prediction du jour courant).
    keep_id : si True, conserve ID_LIEU comme feature (modele global par station).
    scale : si True, applique un RobustScaler aux colonnes continues
            (utile pour la regression lineaire ; inutile pour XGBoost).

    Renvoie
    -------
    (X, y) : X DataFrame de features, y Series cible.
    """
    df = df.copy()
